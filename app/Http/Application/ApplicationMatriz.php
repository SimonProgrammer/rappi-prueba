...
class ApplicationMatriz
{
    ...
    public function sumaCubo($comandos)
    {
        $response = self::procesarComandos($comandos);
        return (count($response) == 0 ? 'Comandos Invalidos' : implode(PHP_EOL,$response));
    }
    public function procesarComandos($comandos){
        $response = array();
        if(self::validarComandos($comandos)){
            foreach ($comandos as $key => $comando) {
                if(count(explode(" ",trim($comando))) == 2){
                   $params = explode(" ",trim($comando));
                   if(is_numeric($params[0])){
                      $this->tam = (int)$params[0];
                      $this->matriz->setMatriz(self::obtenerMatriz());
                   }
                }
                else if(stripos(trim($comando),"UPDATE") !== false){
                    if (!$this->matriz->isEmpty()) {
                       $params = explode(' ',trim($comando));
                       $this->matriz->setPos($params[1],$params[2],$params[3],$params[4]);
                    }
                }
    ...
    public function validarComandos($comandos){
        $valido = true;
        //Validacion T
            ...
            if($valido){
                if($key == 0){
                    ...
                    if(count(explode(" ",trim($comando))) == 2){//linea N M
                        ...
                        if(($key+1) < count($comandos)){
                            if(!(stripos(trim($comandos[($key+1)]),"QUERY") !== false || stripos(trim($comandos[($key+1)]),"UPDATE") !== false)) {
                                $valido = false;
                            }
                        }
                    }
